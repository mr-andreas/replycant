...
## Status
Accepted (amended by [ADR-0009](0009-native-lfs-server-in-gitd.md))
...
Both lookups were pathological against `git-lfs-test-server`, the object store this project previously deployed:
...
- Size lookups issue `GET /objects/{oid}` with `Accept: application/vnd.git-lfs+json`, which the LFS metadata route answers without opening stored content.
- `HEAD` with `Content-Length` remains as a fallback for plain object stores that do not implement the LFS metadata route.
- The `bytes=0-0` range-probe fallback is removed (ADR-0009): the native gitd LFS server returns `Content-Length` on `HEAD` and honors range end bounds, so the probe is no longer needed and was unsafe against pathological upstreams.
